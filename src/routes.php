...
Route::group(['middleware' => \PanicHD\PanicHD\Helpers\LaravelVersion::authMiddleware()], function () use ($main_route, $main_route_path, $admin_route, $admin_route_path) {
    ...
    // Q&A list
    Route::get("$main_route_path/userqnas", 'PanicHD\PanicHD\Controllers\UserQnasController@index')
        ->name("$main_route.userqnas");

    // Other Help
    Route::get("$main_route_path/helpdesk", 'PanicHD\PanicHD\Controllers\TicketsController@indexDirect')
        ->name("$main_route.helpdesk");
    ...
    Route::group(['middleware' => 'PanicHD\PanicHD\Middleware\IsAdminMiddleware'], function () use ($admin_route, $admin_route_path) {
        ...
        //Administrators QnA routes (ex. http://url/panichd/qnacenter)
        Route::resource('qnas','QnasController')->middleware('auth');
        Route::resource("$admin_route_path/qnas", 'PanicHD\PanicHD\Controllers\QnasController', [
            'names' => [
                'index'   => "$admin_route.qnas.index",
                'store'   => "$admin_route.qnas.store",
                'create'  => "$admin_route.qnas.create",
                'update'  => "$admin_route.qnas.update",
                'show'    => "$admin_route.qnas.show",
                'destroy' => "$admin_route.qnas.destroy",
                'edit'    => "$admin_route.qnas.edit",
            ],
        ]);

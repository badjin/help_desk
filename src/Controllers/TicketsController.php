...
class TicketsController extends Controller
{
    ...
    */
    ...
    {
        $tickets = Ticket::inList('active')->visible();
        $count = $tickets->count();
        if (!$this->member->isAdmin() and !$this->member->isAgent() and $count == 0) {
            $qnas = Qna::latest()->get();
            ...
            if ($count == 1)
                return $this->show($tickets->first()->id);
            else
    ...
    */
    ...
    public function create_edit_data($ticket = false, $a_parameters = false)
    {
        ...
        $top_questions = Qna::pluck('question');
...
//                $a_current['cat_id'] = Category::orderBy('name')->first()->id;
                $a_current['cat_id'] = Category::orderBy('id')->first()->id;
            }
        ...
        return compact('menu', 'top_questions', 'c_members', 'priorities', 'status_lists', 'agent_lists', 'a_current', 'permission_level', 'tag_lists', 'a_tags_selected');
    }
    ...
    protected function validation_common($request, $new_ticket = true)
    {
        ...
        $fields = ($permission_level > 1 ? $fields_admin : $fields_user);

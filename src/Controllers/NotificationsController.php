...
//        $adminUser = \PanicHDMember::where('panichd_admin', 1)->first();
//        if ($notification_owner->email != $adminUser) {
//            $a_to[] = [
//                'recipient' => $adminUser,
//                'subject'   => $subject,
//                'template'  => $template,
...
//        }
    ...
    public function ticketStatusUpdated(Ticket $original_ticket, Ticket $ticket, $passFlag)
    {
        $notification_owner = auth()->user();
        ...
        // Notificate assigned agent
        if ($passFlag) $a_to = $this->defaultRecipients($ticket, $notification_owner, $subject, $template);

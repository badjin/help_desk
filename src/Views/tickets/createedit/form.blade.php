...
<div class="card bg-light"><div class="card-body">
    ...
    @endif
        ...
        @if ($u->currentLevel() > 1)
            <div class="jquery_level2_class row" data-class="row"><div class="jquery_level2_class col-md-5" data-class="col-md-5"><!--</div></div>-->
        @endif
...
{{--            ]) !!}--}}
{{--            <div class="{{ $u->currentLevel()==1 ? 'col-lg-10' : 'col-lg-9' }} level_class" data-level-1-class="col-lg-10" data-level-2-class="col-lg-9">'list = "subject"', 'placeholder' => trans('panichd::lang.create-ticket-brief-issue')]) !!}--}}
{{--                <div id="subject_list">--}}
{{--                    <select id="subject" name="subject" class="generate_default_select2 form-control" style="display: none; width: 100%">--}}
{{--                        @if (isset($ticket))--}}
{{--                            <option value="{{ $ticket->subject }}">{{ $ticket->subject }}</option>--}}
{{--                        @else--}}
{{--                            <option value="" disabled selected>Select your question</option>--}}
{{--                        @endif--}}
{{--                        @foreach ($top_questions as $subject)--}}
...
{{--                    </select>--}}
{{--                    <input class="editOption" style="display:none; width: 100%"></input>--}}
{{--                </div>--}}
            ...
            <div class="form-group row" style="margin-bottom: 1.5em"><!-- OWNER -->
                ...
                <div class="{{ $u->currentLevel()==1 ? 'col-lg-10' : 'col-lg-9' }} level_class" data-level-1-class="col-lg-10" data-level-2-class="col-lg-9">
                    ...
                    @foreach ($c_members as $owner)
                        <option value="{{ $owner->id }}" {{ $owner->id == $a_current['owner_id'] ? 'selected="selected"' : '' }}>
                            {{ $owner->name . ($owner->email == "" ? ' ' . trans('panichd::lang.ticket-owner-no-email') : ' - ' . $owner->email) }}
...
{{--            </div>--}}
            ...
            <div class="form-group row align-items-center"><!-- TICKET LIST -->
                <label class="col-lg-3 col-form-label"></label>
{{--                {!! CollectiveForm::label('complete', trans('panichd::lang.status') . trans('panichd::lang.colon'), [--}}
...
{{--                <label class="col-lg-3 col-form-label"></label>--}}
            ...
            <div class="form-group row" style="margin-bottom: 2.5em"><!-- PRIORITY -->
                ...
                <div class="col-lg-9">
                    <div class="input-group date" id="start_date">
                        ...
                        <span class="input-group-append">
                            <button class="btn btn-light btn-default" style="margin-top: -4px"><span class="fa fa-calendar"></span></button>
                        </span>
                    </div>
                    <div class="jquery_error_text"></div>
                </div>
        ...
        @if ($u->currentLevel() > 1)
            </div><div class="jquery_level2_class col-md-7" data-class="col-md-7">
        @endif

        <div class="form-group row"><!-- DESCRIPTION -->
            <label for="content" class="col-lg-3 col-form-label tooltip-info" title="{{ trans('panichd::lang.create-ticket-describe-issue') }}"> *{{trans('panichd::lang.description')}}{{trans('panichd::lang.colon')}} <span class="fa fa-question-circle" style="color: #bbb"></span></label>
            <div class="col-lg-12 summernote-text-wrapper">
            <textarea class="form-control summernote-editor" style="display: none" rows="50" name="content" cols="50">{!! $a_current['description'] !!}</textarea>
            <div class="jquery_error_text"></div>
            </div>
        </div>
...
{{--                    <label for="intervention" class="col-lg-3 col-form-label tooltip-info" title="{{ trans('panichd::lang.create-ticket-intervention-help') }}">{{ trans('panichd::lang.intervention') . trans('panichd::lang.colon') }} <span class="fa fa-question-circle" style="color: #bbb"></span></label>--}}
...
<div class="loading-modal"><h3 class="adapto-centerScreen">Please Wait</h3>
    <div class="progress">
        <div class="progress-bar progress-bar-success myprogress" role="progressbar" style="width:0%"></div>
    </div>
    <span class="percentage"></span>
</div>

{{ Html::script('canvas/js/loading.js') }}
<script>
    function sleep (time) {
        ...
        $(".ajax_form_submit").click(function () {
            sleep(1000).then(() => {
                ...
                if(!flag){
                    ...
                    loaderState.limit = 95;
                    $("body").addClass("loading");
                    dummyProgress(loaderState, 1);
                    showProgressPercentage(loaderState.progress);
                }
            });
        });

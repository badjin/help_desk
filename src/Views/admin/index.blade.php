...
@section('content')
    @if($tickets_count)
        <?php
            $a_cards = [
                ['class' => 'bg-primary', 'icon' => 'book', 'body' => '<h2>' . $tickets_count . '<small>' . trans('panichd::admin.index-total-tickets') . '</small></h2>'],
                ...
                <div class="card bg-light">
                    <div class="card-header">
                      <div class="float-right">
                          <div class="btn-group">
                              <button type="button" class="btn btn-light btn-xs dropdown-toggle" style="margin-top: -7px;" data-toggle="dropdown">
...
//									$counter_lists_text = trans('panichd::lang.newest-tickets-adjective') . ' - ' . trans('panichd::lang.active-tickets-adjective') . ' - ' . trans('panichd::lang.complete-tickets-adjective');
                                    $counter_lists_text = trans('panichd::lang.active-tickets-adjective') . ' - ' . trans('panichd::lang.complete-tickets-adjective');
								?>
                        ...
                        @foreach($categories as $category)
                            @if ($category->name != "Issues")
                                <li class="list-group-item d-flex">
                                    <span style="margin-top: 15px;color: {{ $category->color }}">{{ $category->name }} </span><span class="badge align-self-center ml-1 mr-auto"  style="color: white; background-color: {{ $category->color }}">{{ $category->tickets()->count() }}</span>
...
//                                  $counter_lists_text = trans('panichd::lang.newest-tickets-adjective') . ' - ' . trans('panichd::lang.active-tickets-adjective') . ' - ' . trans('panichd::lang.complete-tickets-adjective');
                        ...
                        @foreach($agents as $agent)
                            ...
                            <?php $agent_text = $agent->name . ' <span class="badge">' . $agent->ticketsAsAgent()->count() . '</span>'; ?>
                             <span class="mr-auto" style="margin-top: 15px">
